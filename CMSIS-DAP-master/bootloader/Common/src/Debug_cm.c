  ...
  unsigned long address = 0x20002000;
	...
	{		
		...
		for(i = 0; i < RETRY_TIME; i++)
		{
			...
			t_read_mem(0,FLASH_PC - 1, 8, 1024, (unsigned char *)pread);
		...
		for(i = 0; i < filenb;i++)
		{
			for(j = 0; j < (sectorsize/rambufsize); j++)
			{
				...
				{
					write_error = 0;
					pwrite = (uint32_t *)(START_APP_ADDRESS + 0x400*i + 0x100*j);
					pread = (uint32_t *)READBACK_ADDR;
					t_write_mem(0,HWBUF_DEFAULT, 8, 256, (unsigned char *)pwrite);
					t_read_mem(0,HWBUF_DEFAULT, 8, 256, (unsigned char *)pread);
